...
from roofmapnet.utils import argsort2d
...
def APJ(vert_pred, vert_gt, max_distance, im_ids):
    ...
    vert_gt = np.array(vert_gt, dtype=object)
    ...
    nd = len(im_ids)
    tp, fp = np.zeros(nd, dtype=np.float64), np.zeros(nd, dtype=np.float64)
...
def nms_j(heatmap, delta=1):
    ...
    disable = np.zeros_like(heatmap, dtype=np.bool_)
